...
let spellLists, featsList, traitsList, languagesList, day_list, known_list;
function updateCharacterDescription() {
  ...
  .then(characterData => {
      ...
      // Auto Replace Skills function
        ...
        //   console.log("known_list", known_list);                    
      } else {
          console.log("spellLists is not populated");
          // Handle the case where spellLists is not populated
      }

        // Autofill dropdown menu for alignment
        const alignmentDropdown = document.getElementById('basics-character-alignment');
        if (characterData.alignment) {
            alignmentDropdown.value = characterData.alignment;
    }  

            // Autofill dropdown menu for alignment
        const sizeDropdown = document.getElementById('basics-character-size-category');
        const largeRaces = ['Tortugan', 'Loxophant', 'Half-Giant', 'half-giant', 'Half-giant'];
        if (largeRaces.includes(characterData.race)) {
            sizeDropdown.value = "Large";
        } else {
    ...
    //   Need to add a fix for Perform + knowledge martial + Profession portions + remove skills that don't belong in the list
      ...
      const keysToDisplay = ['background_traits', 'flaws', 'professions', 'mannerisms', 'hair_color', 'hair_type', 'eye_color', 'appearance'];
      let descriptionHTML = '';
      
      keysToDisplay.forEach(key => {
          if (characterData.hasOwnProperty(key)) {
              const values = characterData[key].join(', ');
              descriptionHTML += `<pre>${key.replace('_', ' ')}: ${values}</pre>`;
          }
      });
      
      characterDescriptionElement.innerHTML = descriptionHTML;
...
if (updateDescriptionButton) {
    ...
    async function handleSpellsAndFeats() {
        ...
        for (let i = 0; i < traitsList.length; i++) {
            ...
            console.log(`LanguageTrait Name: ${traitName}`);
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            if (traitInput && addButton && traitName) {
                ...
                existingFeats.forEach(existingFeat => {
                    if (existingFeat.textContent.trim() === traitName) {
    ...
    return new Promise(resolve => {
        setTimeout(() => {
            ...
            if (languageInput && addButton && languageName) {
                ...
                existingFeats.forEach(existingFeat => {
                    if (existingFeat.textContent.trim() === languageName) {
                ...
                if (!languageAlreadyExists) {
                    ...
                    console.log('Trait already exists:', languageName);
                    resolve();
                }
            }
        }, 200); // Adjust delay as needed
